...
(function() {
    ...
    function domToNode(domNode) {
        ...
        if (domNode.childNodes.length > 0) {
            ...
            for (let i = 0; i < domNode.childNodes.length; i++) {
                ...
                let converted = domToNode(child);
                if (converted) nodeElement.children.push(converted);
            }
        }
        return nodeElement;
    }
        ...
        if (node.tag) {
            ...
            if (node.attrs) {
                for (let name in node.attrs) {
                    domNode.setAttribute(name, node.attrs[name]);
                }
            }
        } else {
    ...
    function initGallery() {
        ...
        function telegraphCreatePage(title, htmlContent) {
            getTelegraphToken().then(token => {
                ...
                // Convert HTML → Telegraph JSON nodes
                let container = document.createElement("div");
                container.id = "article";
                container.innerHTML = htmlContent;
                let content = domToNode(container).children;
